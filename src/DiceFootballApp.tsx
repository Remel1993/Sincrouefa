...
function DiceFootballApp() {
  ...
  const applyCareerMatchday = (
    ...
    const sortedBefore = [...currentTeams].sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf);
    ...
    const matchdayResults = round.map(m => {
      if (m.homeId === careerFixture.homeId && m.awayId === careerFixture.awayId) {
        return { hId: m.homeId, aId: m.awayId, sh: scoreH, sa: scoreA };
      }
      const h = currentTeams.find(t => t.id === m.homeId);
      const a = currentTeams.find(t => t.id === m.awayId);
      const { sh, sa } = simMatchGoals(h?.opp, h?.att, a?.def, a?.opp, a?.att, h?.def);
      return { hId: m.homeId, aId: m.awayId, sh, sa };
    });

    const updatedTeams = currentTeams.map(t => {
      const res = matchdayResults.find(r => r.hId === t.id || r.aId === t.id);
    ...
    const sortedAfter = [...updatedTeams].sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf);
    const posAfterIdx = sortedAfter.findIndex(t => t.id === career.teamId);
    const posAfter = posAfterIdx >= 0 ? posAfterIdx + 1 : posBefore;
    ...
    setComps(prev => {
      ...
      return {
        ...
        [compId]: {
          ...
          [careerHistKey]: [{ day: careerMd + 1, results: matchdayResults }, ...(comp[careerHistKey] || [])],
  ...
  const simulateCareerMatchday = (advanceSeasonWeek: boolean = true) => {
    ...
    const userPendingUel = hasEuropa && isCareerAliveInC3 && ((comps['C3']?.matchday || 0) < (expUelMd ?? 99)) && currentWk < 42;

    // Si confluyen compromisos europeos pendientes esta semana, simular de forma simultánea
    if (userPendingCl || userPendingUel) {
      simulateSeasonWeek();
      return;
    }
  ...
  const simulateSeasonWeek = () => {
    ...
    // Preparación y resolución simultánea de los compromisos del mánager en esta semana
    ...
    // 1. Partido de Liga del mánager (si confluye esta semana)
    let managerRoundResults: any[] | null = null;
    let managerUpdatedTeams: any[] | null = null;

    if (userPendingLeague && careerTeam && careerRival) {
      const baseStats = {
        att: Math.max(career.baseDist?.att || 1, careerTeam.att || 1),
        opp: Math.max(career.baseDist?.opp || 1, careerTeam.opp || 1),
        def: Math.max(career.baseDist?.def || 1, careerTeam.def || 1)
      };
      const tactic = career.tactic ? { ...career.tactic } : { ...baseStats };
      const mine = { ...careerTeam, att: tactic.att, opp: tactic.opp, def: tactic.def };
      const home = careerIsHome ? mine : careerRival;
      const away = careerIsHome ? careerRival : mine;
      const { sh, sa } = simMatchGoals(home.opp, home.att, away.def, away.opp, away.att, home.def);
      const myGf = careerIsHome ? sh : sa;
      const myGa = careerIsHome ? sa : sh;
      const res = myGf > myGa ? 'W' : myGf === myGa ? 'D' : 'L';
      const matchPe = res === 'W' ? 3 : res === 'D' ? 1 : 0;
      const bonusPE = (career.tier <= 2 && (careerRival.att + careerRival.opp + careerRival.def) >= 12 && res === 'W') ? 2 : 0;
      const leaguePe = matchPe + bonusPE;
      const ownStr = baseStats.att + baseStats.opp + baseStats.def;
      const rivalStr = careerRival.att + careerRival.opp + careerRival.def;
      const rep = repForMatch(res, ownStr, rivalStr, career.reputation || 10);

      totalSimPe += leaguePe;
      totalSimRep += rep;
      matchesCount += 1;
      goalsForCount += myGf;
      goalsAgainstCount += myGa;
      if (res === 'W') winsCount += 1;
      else if (res === 'D') drawsCount += 1;
      else lossesCount += 1;
      ...
      // Posición previa y posterior calculada de forma consistente y sincrónica
      const currentComp = comps[career.compId];
      const currentTeams = currentComp ? (career.div === 2 ? currentComp.teams2 : currentComp.teams) || [] : [];
      const sortedBefore = [...currentTeams].sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf);
      const posBeforeIdx = sortedBefore.findIndex(t => t.id === career.teamId);
      const leaguePosBefore = posBeforeIdx >= 0 ? posBeforeIdx + 1 : 1;

      const round = careerSchedule[careerMd] || [];
      managerRoundResults = round.map(m => {
        if (m.homeId === careerFixture.homeId && m.awayId === careerFixture.awayId) {
          return { hId: m.homeId, aId: m.awayId, sh, sa };
        }
        const h = currentTeams.find(t => t.id === m.homeId);
        const a = currentTeams.find(t => t.id === m.awayId);
        const { sh: mSh, sa: mSa } = simMatchGoals(h?.opp, h?.att, a?.def, a?.opp, a?.att, h?.def);
        return { hId: m.homeId, aId: m.awayId, sh: mSh, sa: mSa };
      });

      managerUpdatedTeams = currentTeams.map(t => {
        const resMatch = managerRoundResults!.find(r => r.hId === t.id || r.aId === t.id);
        if (!resMatch) return t.id === career.teamId ? { ...t, ...baseStats } : t;
        const isH = resMatch.hId === t.id;
        const gf = isH ? resMatch.sh : resMatch.sa;
        const ga = isH ? resMatch.sa : resMatch.sh;
        const w = gf > ga ? 1 : 0;
        const d = gf === ga ? 1 : 0;
        const l = gf < ga ? 1 : 0;
        return {
          ...t,
          ...(t.id === career.teamId ? baseStats : {}),
          p: (t.p || 0) + 1,
          w: (t.w || 0) + w,
          d: (t.d || 0) + d,
          l: (t.l || 0) + l,
      ...
      const sortedAfter = [...managerUpdatedTeams].sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf);
      const posAfterIdx = sortedAfter.findIndex(t => t.id === career.teamId);
      const leaguePosAfter = posAfterIdx >= 0 ? posAfterIdx + 1 : leaguePosBefore;
      ...
      leagueMatchFeedback = {
        ...
        myGa,
        result: res,
        posBefore: leaguePosBefore,
        posAfter: leaguePosAfter,
        peGained: leaguePe,
        repGained: rep
      };
    }

    // 2. Partido de Champions League del mánager (si confluye esta semana)
    if (userPendingCl && careerClTeam && careerClInfo?.rivalTeam) {
      ...
      totalSimRep += clRep;
      matchesCount += 1;
      goalsForCount += myClGf;
      goalsAgainstCount += myClGa;
      if (clRes === 'W') winsCount += 1;
      else if (clRes === 'D') drawsCount += 1;
      else lossesCount += 1;
      ...
      clMatchFeedback = {
        ...
        myGa: myClGa,
        result: clRes,
        phase: careerClInfo.phaseLabel || 'Champions League',
        peGained: clPe,
        repGained: clRep
      };
    }

    // 3. Partido de UEFA Europa League del mánager (si confluye esta semana)
    if (userPendingUel && careerUelTeam && careerUelInfo?.rivalTeam) {
      ...
      totalSimRep += uelRep;
      matchesCount += 1;
      goalsForCount += myUelGf;
      goalsAgainstCount += myUelGa;
      if (uelRes === 'W') winsCount += 1;
      else if (uelRes === 'D') drawsCount += 1;
      else lossesCount += 1;
      ...
      uelMatchFeedback = {
        ...
        myGa: myUelGa,
        result: uelRes,
        phase: uelPhaseLabel(careerUelInfo.phase),
        peGained: uelPe,
        repGained: uelRep
      };
    }
    ...
    setComps(prev => {
      ...
      if (hasLeague || !weekData) {
        ...
        LEAGUE_IDS.forEach(compId => {
          ...
          if (userPendingLeague && compId === career.compId && managerUpdatedTeams && managerRoundResults) {
            const isTargetDiv2 = career.div === 2;
            const teamsKey = isTargetDiv2 ? 'teams2' : 'teams';
            const mdKey = isTargetDiv2 ? 'matchday2' : 'matchday';
            const histKey = isTargetDiv2 ? 'history2' : 'history';
            const winKey = isTargetDiv2 ? 'showWinner2' : 'showWinner';
            const isFin = careerMd + 1 >= (careerSchedule?.length || 38);

            upd = {
              ...upd,
              [teamsKey]: managerUpdatedTeams,
              [mdKey]: careerMd + 1,
              [histKey]: [{ day: careerMd + 1, results: managerRoundResults }, ...(upd[histKey] || [])],
              [winKey]: isFin ? true : upd[winKey]
            };

            // Simular la otra división de esta liga si correspondía
            if (isTargetDiv2) {
              runDiv('teams', 'matchday', 'history', 'showWinner', false);
            } else {
              runDiv('teams2', 'matchday2', 'history2', 'showWinner2', true);
    ...
    // Aplicar al mánager las recompensas, estadísticas y feedback acumulados de los partidos simulados esta semana
    if (matchesCount > 0) {
      setCareer(c => {
        if (!c.active) return c;
        const prevStats = c.stats || { p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 };
        const newStats = {
          ...prevStats,
          p: (prevStats.p || 0) + matchesCount,
          w: (prevStats.w || 0) + winsCount,
          d: (prevStats.d || 0) + drawsCount,
          l: (prevStats.l || 0) + lossesCount,
          gf: (prevStats.gf || 0) + goalsForCount,
          ga: (prevStats.ga || 0) + goalsAgainstCount,
          pts: (prevStats.pts || 0) + (winsCount * 3 + drawsCount)
        };
        ...
        const isSimultaneous = Boolean(leagueMatchFeedback && (clMatchFeedback || uelMatchFeedback));
        const simFeedback = {
          isSimultaneous,
          matchday: currentWk,
          headline: isSimultaneous ? `Resultados Simultáneos · Semana ${currentWk}` : undefined,
          leagueMatch: leagueMatchFeedback || undefined,
          clMatch: clMatchFeedback || undefined,
          uelMatch: uelMatchFeedback || undefined,
          isChampions: !leagueMatchFeedback && Boolean(clMatchFeedback),
          isEuropaLeague: !leagueMatchFeedback && Boolean(uelMatchFeedback),
          phaseLabel: clMatchFeedback?.phase || uelMatchFeedback?.phase,
          posBefore: leagueMatchFeedback ? leagueMatchFeedback.posBefore : undefined,
          posAfter: leagueMatchFeedback ? leagueMatchFeedback.posAfter : undefined,
          expectedPos: expPos,
          targetPos: tgtPos,
          peGained: totalSimPe,
          repGained: Number(totalSimRep.toFixed(2)),
          rivalName: leagueMatchFeedback ? leagueMatchFeedback.rivalName : (clMatchFeedback ? clMatchFeedback.rivalName : uelMatchFeedback?.rivalName),
          myGf: leagueMatchFeedback ? leagueMatchFeedback.myGf : (clMatchFeedback ? clMatchFeedback.myGf : uelMatchFeedback?.myGf),
          myGa: leagueMatchFeedback ? leagueMatchFeedback.myGa : (clMatchFeedback ? clMatchFeedback.myGa : uelMatchFeedback?.myGa),
          result: leagueMatchFeedback ? leagueMatchFeedback.result : (clMatchFeedback ? clMatchFeedback.result : uelMatchFeedback?.result),
          summary: isSimultaneous
            ? `Semana ${currentWk} resuelta simultáneamente: ${leagueMatchFeedback ? 'Liga Regular (' + (leagueMatchFeedback.result === 'W' ? 'Victoria' : leagueMatchFeedback.result === 'D' ? 'Empate' : 'Derrota') + ')' : ''} ${clMatchFeedback ? '+ Champions League (' + (clMatchFeedback.result === 'W' ? 'Victoria' : clMatchFeedback.result === 'D' ? 'Empate' : 'Derrota') + ')' : ''} ${uelMatchFeedback ? '+ Europa League (' + (uelMatchFeedback.result === 'W' ? 'Victoria' : uelMatchFeedback.result === 'D' ? 'Empate' : 'Derrota') + ')' : ''}.`
            : leagueMatchFeedback
            ? `Jornada de Liga ${careerMd + 1} resuelta: ${leagueMatchFeedback.result === 'W' ? 'Victoria' : leagueMatchFeedback.result === 'D' ? 'Empate' : 'Derrota'} ${leagueMatchFeedback.myGf}-${leagueMatchFeedback.myGa} ante ${leagueMatchFeedback.rivalName}.`
            : clMatchFeedback
            ? `Champions League (${clMatchFeedback.phase}): ${clMatchFeedback.result === 'W' ? 'Victoria' : clMatchFeedback.result === 'D' ? 'Empate' : 'Derrota'} ${clMatchFeedback.myGf}-${clMatchFeedback.myGa} ante ${clMatchFeedback.rivalName}.`
            : uelMatchFeedback
            ? `Europa League (${uelMatchFeedback.phase}): ${uelMatchFeedback.result === 'W' ? 'Victoria' : uelMatchFeedback.result === 'D' ? 'Empate' : 'Derrota'} ${uelMatchFeedback.myGf}-${uelMatchFeedback.myGa} ante ${uelMatchFeedback.rivalName}.`
            : `Semana ${currentWk} simulada con éxito.`
        };
        ...
        return {
          ...
          stats: newStats,
          lastSimulationFeedback: simFeedback,
          seasonLog: [...newLogs, ...(c.seasonLog || [])].slice(0, 60)
        };
      });
    }
  ...
  const processCupRound = (ms?: any, targetCompId?: string, isAutoSimManual?: boolean) => {
    ...
    // Verificación estricta de calendario para copas europeas (C1 y C3) cuando es simulación automática
    if (!ms && (cId === 'C1' || cId === 'C3')) {
      const currentWk = seasonState.currentWeek || 1;
